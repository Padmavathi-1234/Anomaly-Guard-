...
for i in range(NUM_SYNTH):
    ...
    level = random.randint(1, min(4, 1 + i // 30))
    ...
    if i % 3 != 0:
        ...
        scenario = scenario_gen.generate(
            difficulty=diff, seed=seed + 1, curriculum_level=level
        )
    ...
    if i % 4 == 0:
        ...
        topo_ctx = (
            f"\nNetwork: {topo['segment_count']} segments, "
            f"{topo['host_count']} hosts."
        )
    ...
    action = {
        ...
        "justification": {
            ...
            "alternatives_considered": [
                {
                    "action":           "monitor",
                    "rejected_because": "Active threat requires immediate response",
                }
            ],
        },
    }
...
def reward_func(completions: List[str], **kwargs) -> List[float]:
    ...
    for completion in completions:
        ...
        fmt = 0.0
        for key in (
            "action_type", "target_id", "justification",
            "reasoning", "risk_assessment", "alternatives_considered",
        ):
            if key in text:
                fmt += 0.12
        fmt = min(fmt, 0.7)
        ...
        lower    = text.lower()
        for kw in (
            "suspicious", "anomalous", "malicious", "evidence",
        ...
        hack_action_hist = [{
            "action_type": parsed.get("action_type", "monitor"),
    ...
    anti_hack_log.append({
        "step":  call_counter,
        "batch": batch_anti_hack[:2],
    })
...
def rollout_function(prompts: List[str]) -> List[Dict]:
    ...
    for prompt in prompts:
        ...
        while steps < ROLLOUT_EP_LEN:
            ...
            obs, rewards_dict, terminated, truncated, step_info = env.step(
                multi_action
            )
            triage_obs = (
                obs.get(AgentRole.TRIAGE, obs)
            ...
            triage_reward = (
                rewards_dict.get(AgentRole.TRIAGE, 0.0)
                if isinstance(rewards_dict, dict)
                else float(rewards_dict)
            )

            _, ah_penalty, _ = anti_hack.check(
                ep_actions,
                parsed,
                triage_obs if isinstance(triage_obs, dict) else {},
            )
            triage_reward += ah_penalty

            comp = (
                step_info.get("compliance", {})
                         .get(AgentRole.TRIAGE, {})
            )
            if comp:
    ...
    log.info(
        "Rollout batch: min=%.3f max=%.3f avg=%.3f | curriculum_level=%d",
        min(rews), max(rews),
        sum(rews) / len(rews),
...
def _parse_action(text: str) -> Dict:
    ...
    return {
        ...
        "target_id":   "host-01",
        "justification": {
            "reasoning": "Default safe investigation action"
        },
    }
...
def main():
    ...
    log.info("  Curriculum  : 8 levels, starting at L%d",
             curriculum.current_level)
    log.info("  Subsystems  : RealisticScenario, ProceduralAttack,")
    log.info("                Topology, LiveThreatIntel,")
    log.info("                MultiComponentReward, AntiHack,")
...
def _generate_plots():
    ...
    ax.plot(
        step_log, reward_log,
        "o-", lw=2, ms=5,
        color="#00A67E", label="GRPO Agent",
    )
    ax.axhline(
        y=0.787, color="red", ls="--", lw=2,
        label="Rule-based Baseline (0.787)",
    )
    ax.fill_between(step_log, 0.787, reward_log,
                    alpha=0.15, color="#00A67E")
    if reward_log and reward_log[-1] > 0.787:
        imp = reward_log[-1] - 0.787
        ax.annotate(
            f"+{imp:.2f} ({imp/0.787*100:.0f}%)",
            xy=(step_log[-1], reward_log[-1]),
            xytext=(step_log[-1] - 20, reward_log[-1] - 0.08),
            arrowprops=dict(
                facecolor="black", shrink=0.05, width=1.5
            ),
    ...
    ax.axvline(
        np.mean(reward_log), color="red", ls="--", lw=2,
        label=f"Mean={np.mean(reward_log):.3f}",
    )
